...
public class RedFor5DaysAnalyst {
	...
	public static void main(String[] args) {
		...
		String sql = "SELECT stockid, ztgs FROM ( "
				+ " SELECT stockid, COUNT(zdf) ztgs FROM stockdailyinfo WHERE zdf > 0 AND highestprice != lowestprice AND todayopenprice != 0  AND DATE > '"
				+ DateUtil.formatDay(weeksAgo)
				+ "' and Date < '"
				+ DateUtil.formatDay(next)
				+ "' GROUP BY stockid) a WHERE a.ztgs >= 8 ORDER BY a.ztgs DESC ";
